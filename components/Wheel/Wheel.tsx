...
const Wheel: React.FC<IWheelProps> = ({ teamNames, canvasSize }) => {
  ...
  const animateSpin = useCallback((startTime: number, endTime: number) => {
    ...
    const animationLoop = (prevRotation: number = 0) => {
      ...
      if (!timeUp) {
        const rotationAmount =
          1 -
          easeOut(
            calculateTimeProgress(performance.now(), startTime, endTime)
          );
  ...
  return (
    <Flex flexDirection="column" placeItems="center">
      ...
      {landedTeam && atLeastOneSpin && !spinning && <Heading my={4}>{ landedTeam } was eliminated</Heading>}
    </Flex>
  );
};
